...
class Config:
    ...
    # [R mode] 측정노이즈 R 결정 방식 (covariance/FV 전용). use_adaptive_r는 __post_init__에서 이로부터 파생.
    #   'fixed'      = R = r_init 스케줄값 (고정)
    #   'adaptive'   = R = max(R_min, λ·Tr(P_zz_sigma)/n_d)  (처방 A — sigma-spread/UT alpha 의존, 환경 따라 폭발)
    #   'innovation' = R = R_min + β·mean(innovation²)        (실제 잔차 |y−Q| 기반 — alpha 무관, 후반 gain↓는 X)
    #   'ratio'      = R = R_min + λ·Tr(P_zz_sigma)/max(Var(innovation), ε)  (adaptive ÷ Var(innov);
    #                  H 성장 반영 + Q²/H² 상쇄(환경 무관) + 수렴 시 R↑→gain↓. ← "후반 gain 감쇠" 목표용)
    r_mode: str = 'ratio'
    innov_r_beta: float = 1.5       # innovation 모드 β: R = R_min(=adaptive_r_min) + β·mean(residual²)
    innov_var_eps: float = 1e-2     # ratio 모드 ε: max(Var(innov), ε) 분모 하한 (완전 수렴 시 R 폭발 방지·gain↓ 상한 결정)

    use_adaptive_r: bool = False    # [파생] r_mode != 'fixed' 이면 True (직접 설정  — r_mode가 진실원천)
    adaptive_r_lambda: float = 1    # λ: Tr(HPH^T)/n_d에 곱하는 신뢰도 스케일 (adaptive/ratio 모드)
    adaptive_r_min: float = 0.5    # R_min: 게인 폭주 방지 하한 (분산 단위). innovation/ratio R_min도 공유.
...
def compute_recovery_metric(rewards, cfg):
    ...
    base_hi = max(0, a - 1)
    if base_hi <= base_lo:
        return {'baseline': None, 'dip_min': None, 'recovery_ep': None,
